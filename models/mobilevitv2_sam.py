# dual-import so it works as a package (-m) or by absolute path
try:
...
class MobileViTv2_SAM(nn.Module):
    """
    Wraps a MobileViT-v2 (timm) backbone, applies SAM on the final feature map,
...
def get_mobilevitv2_sam(num_classes: int):
    return MobileViTv2_SAM(num_classes=num_classes)
